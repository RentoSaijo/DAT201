...
{\centering \includegraphics{Paper_files/figure-latex/fig-movement-1} 

}

\caption{Within every shot-distance band, the highest on-net rate comes from plays with at least 25 feet of lateral puck movement before the shot. Facets show shot-distance groups. Sample restricted to 5-on-5 shots preceded by a completed same-team play within five seconds (n = 655).}\label{fig:fig-movement}
\end{figure}
...
where \(p_i\) is the probability that shot \(i\) finished on net, \(\text{ShotDistance}_i\) is the shot's distance from the goal, \(\text{AbsY}_i\) is its absolute side-to-side location, and \(\text{LateralMovement}_i\) is the pre-shot east-west puck movement measured in 10-foot units. In the fitted model, the movement coefficient is \(\hat{\beta}_3 = 0.117\), while the shot-distance coefficient is \(\hat{\beta}_1 = -0.030\). So, holding the other terms fixed, each additional 10 feet of lateral pre-shot movement increased the odds that a shot ended on net by about 12.5\%, whereas shots taken farther from the net became less likely to finish on net. The categorical version of the same model shows that the 25+ foot group had about 1.53 times the on-net odds of the 0-10 foot group. This does not mean lateral movement explains every good chance, but it does mean the process leading into the shot carries independent information that a shot-only model can miss.
